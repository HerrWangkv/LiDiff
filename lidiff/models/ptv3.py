# Adapted from https://github.com/Pointcept/PointTransformerV3/blob/main/model.py
...
class Block(PointModule):
    ...
    def get_time_embedding(self, timesteps):
        ...
        if self.channels % 2 == 1:  # zero pad
            emb = nn.functional.pad(emb, (0, 1), "constant", 0)
        assert emb.shape == (timesteps.shape[0], self.channels)
        return self.te(emb)

    def forward(self, point: Point, t: torch.Tensor=None):
        shortcut = point.feat
        point = self.cpe(point)
        point.feat = shortcut + point.feat
        if t is not None:
            temp_emb = self.get_time_embedding(t)
            batch_temp = torch.unique(point.batch, return_counts=True)[1]
            temp_emb = temp_emb.repeat_interleave(batch_temp, dim=0)
            point.feat += temp_emb
        shortcut = point.feat
        if self.pre_norm:
...
class SerializedPooling(PointModule):
    ...
    def forward(self, point: Point, t=None):
...
class PointTransformerV3(PointModule):
    ...
    def forward(self, data_dict, t):
        ...
        point = self.enc(point, t)
        if not self.cls_mode:
            point = self.dec(point, t)
            point = self.output_head(point.feat)
        return point
